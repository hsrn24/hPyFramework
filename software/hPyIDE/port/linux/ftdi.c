...
/**
    \mainpage libftdi API documentation
		...
		Library to talk to m_ftdi chips. You find the latest versions of libftdi at
...
/**
    Internal function to close usb device pointer.
		Sets m_ftdi->usb_dev to NULL.
    \internal

		\param m_ftdi pointer to ftdi_context

    \retval none
*/
static void ftdi_usb_close_internal (struct ftdi_context *m_ftdi)
{
		if (m_ftdi && m_ftdi->usb_dev)
    {
				libusb_close (m_ftdi->usb_dev);
				m_ftdi->usb_dev = NULL;
				if(m_ftdi->eeprom)
						m_ftdi->eeprom->initialized_for_connected_device = 0;
    }
}
...
*/
int ftdi_init(struct ftdi_context *m_ftdi)
{
    struct ftdi_eeprom* eeprom = (struct ftdi_eeprom *)malloc(sizeof(struct ftdi_eeprom));
		...
		if (libusb_init(&m_ftdi->usb_ctx) < 0)
    ...
    memset(eeprom, 0, sizeof(struct ftdi_eeprom));
		m_ftdi->eeprom = eeprom;

    /* All fine. Now allocate the readbuffer */
		return ftdi_read_data_set_chunksize(m_ftdi, 4096);
}
...
{
		struct ftdi_context * m_ftdi = (struct ftdi_context *)malloc(sizeof(struct ftdi_context));
...
*/
int ftdi_set_interface(struct ftdi_context *m_ftdi, enum ftdi_interface interface)
{
		if (m_ftdi == NULL)
        ftdi_error_return(-2, "USB device unavailable");

		if (m_ftdi->usb_dev != NULL)
    {
        int check_interface = interface;
        if (check_interface == INTERFACE_ANY)
            check_interface = INTERFACE_A;

				if (m_ftdi->index != check_interface)
...
/**
    Deinitializes a ftdi_context.
		...
		\param m_ftdi pointer to ftdi_context
*/
void ftdi_deinit(struct ftdi_context *m_ftdi)
{
		if (m_ftdi == NULL)
        return;

		ftdi_usb_close_internal (m_ftdi);

		if (m_ftdi->readbuffer != NULL)
    {
				free(m_ftdi->readbuffer);
				m_ftdi->readbuffer = NULL;
    }

		if (m_ftdi->eeprom != NULL)
    {
				if (m_ftdi->eeprom->manufacturer != 0)
        {
						free(m_ftdi->eeprom->manufacturer);
						m_ftdi->eeprom->manufacturer = 0;
        }
				if (m_ftdi->eeprom->product != 0)
        {
						free(m_ftdi->eeprom->product);
						m_ftdi->eeprom->product = 0;
        }
				if (m_ftdi->eeprom->serial != 0)
        {
						free(m_ftdi->eeprom->serial);
						m_ftdi->eeprom->serial = 0;
        }
				free(m_ftdi->eeprom);
				m_ftdi->eeprom = NULL;
    }
...
/**
    Deinitialize and free an ftdi_context.
		...
		\param m_ftdi pointer to ftdi_context
*/
void ftdi_free(struct ftdi_context *m_ftdi)
{
		ftdi_deinit(m_ftdi);
		free(m_ftdi);
}
...
*/
void ftdi_set_usbdev (struct ftdi_context *m_ftdi, libusb_device_handle *usb)
{
		if (m_ftdi == NULL)
        return;
...
/**
		Finds all m_ftdi devices with given VID:PID on the usb bus. Creates a new
...
*/
int ftdi_usb_find_all(struct ftdi_context *m_ftdi, struct ftdi_device_list **devlist, int vendor, int product)
{
    ...
    int i = 0;
		...
		if (libusb_get_device_list(m_ftdi->usb_ctx, &devs) < 0)
...
*/
int ftdi_usb_get_strings(struct ftdi_context * m_ftdi, struct libusb_device * dev,
                         char * manufacturer, int mnf_len, char * description, int desc_len, char * serial, int serial_len)
{
    struct libusb_device_descriptor desc;

		if ((m_ftdi==NULL) || (dev==NULL))
        return -1;

		if (m_ftdi->usb_dev == NULL && libusb_open(dev, &m_ftdi->usb_dev) < 0)
    ...
    {
				if (libusb_get_string_descriptor_ascii(m_ftdi->usb_dev, desc.iManufacturer, (unsigned char *)manufacturer, mnf_len) < 0)
        {
						ftdi_usb_close_internal (m_ftdi);
    ...
    if (description != NULL)
    {
				if (libusb_get_string_descriptor_ascii(m_ftdi->usb_dev, desc.iProduct, (unsigned char *)description, desc_len) < 0)
        {
						ftdi_usb_close_internal (m_ftdi);
    ...
    if (serial != NULL)
    {
				if (libusb_get_string_descriptor_ascii(m_ftdi->usb_dev, desc.iSerialNumber, (unsigned char *)serial, serial_len) < 0)
        {
						ftdi_usb_close_internal (m_ftdi);
 ...
 */
static unsigned int _ftdi_determine_max_packet_size(struct ftdi_context *m_ftdi, libusb_device *dev)
{
    struct libusb_device_descriptor desc;
    ...
    // Determine maximum packet size. Init with default value.
		// New hi-speed devices from m_ftdi use a packet size of 512 bytes
    ...
    {
				if (m_ftdi->interface < config0->bNumInterfaces)
        {
						struct libusb_interface interface = config0->interface[m_ftdi->interface];
...
/**
		Opens a m_ftdi device given by an usb_device.
    ...
    \retval -7: set baudrate failed
		\retval -8: m_ftdi context invalid
...
*/
int ftdi_usb_open_dev(struct ftdi_context *m_ftdi, libusb_device *dev)
{
    struct libusb_device_descriptor desc;
    struct libusb_config_descriptor *config0;
    int cfg, cfg0, detach_errno = 0;
		...
		if (m_ftdi == NULL)
				ftdi_error_return(-8, "m_ftdi context invalid");

		if (libusb_open(dev, &m_ftdi->usb_dev) < 0)
        ftdi_error_return(-4, "libusb_open() failed");
    ...
    // Likely scenario is a static ftdi_sio kernel module.
		if (m_ftdi->module_detach_mode == AUTO_DETACH_SIO_MODULE)
    {
				if (libusb_detach_kernel_driver(m_ftdi->usb_dev, m_ftdi->interface) !=0)
            detach_errno = errno;
    }

		if (libusb_get_configuration (m_ftdi->usb_dev, &cfg) < 0)
    ...
    {
				if (libusb_set_configuration(m_ftdi->usb_dev, cfg0) < 0)
        {
						ftdi_usb_close_internal (m_ftdi);
            ...
            {
								ftdi_error_return(-3, "unable to set usb configuration. Make sure the default m_ftdi driver is not in use");
            }
        }
    }

		if (libusb_claim_interface(m_ftdi->usb_dev, m_ftdi->interface) < 0)
    {
				ftdi_usb_close_internal (m_ftdi);
        if (detach_errno == EPERM)
        {
        ...
        {
						ftdi_error_return(-5, "unable to claim usb device. Make sure the default m_ftdi driver is not in use");
        }
    }
    ...
    if (desc.bcdDevice == 0x400 || (desc.bcdDevice == 0x200
				...
				m_ftdi->type = TYPE_2232C;
    else if (desc.bcdDevice == 0x600)
				m_ftdi->type = TYPE_R;
    else if (desc.bcdDevice == 0x700)
				m_ftdi->type = TYPE_2232H;
    else if (desc.bcdDevice == 0x800)
				m_ftdi->type = TYPE_4232H;
    else if (desc.bcdDevice == 0x900)
				m_ftdi->type = TYPE_232H;
    else if (desc.bcdDevice == 0x1000)
				m_ftdi->type = TYPE_230X;

    // Determine maximum packet size
		m_ftdi->max_packet_size = _ftdi_determine_max_packet_size(m_ftdi, dev);

		if (ftdi_set_baudrate (m_ftdi, 9600) != 0)
    {
				ftdi_usb_close_internal (m_ftdi);
...
*/
int ftdi_usb_open(struct ftdi_context *m_ftdi, int vendor, int product)
{
		return ftdi_usb_open_desc(m_ftdi, vendor, product, NULL, NULL);
}
...
*/
int ftdi_usb_open_desc(struct ftdi_context *m_ftdi, int vendor, int product,
                       const char* description, const char* serial)
{
		return ftdi_usb_open_desc_index(m_ftdi,vendor,product,description,serial,0);
}
...
/**
    ...
    \retval -10: unable to close device
		\retval -11: m_ftdi context invalid
*/
int ftdi_usb_open_desc_index(struct ftdi_context *m_ftdi, int vendor, int product,
                             const char* description, const char* serial, unsigned int index)
{
    ...
    int i = 0;
		...
		if (m_ftdi == NULL)
				ftdi_error_return(-11, "m_ftdi context invalid");
    ...
    while ((dev = devs[i++]) != NULL)
    {
        ...
        if (desc.idVendor == vendor && desc.idProduct == product)
        {
            ...
            if (description != NULL)
            {
								if (libusb_get_string_descriptor_ascii(m_ftdi->usb_dev, desc.iProduct, (unsigned char *)string, sizeof(string)) < 0)
                {
										ftdi_usb_close_internal (m_ftdi);
            ...
            if (serial != NULL)
            {
								if (libusb_get_string_descriptor_ascii(m_ftdi->usb_dev, desc.iSerialNumber, (unsigned char *)string, sizeof(string)) < 0)
                {
										ftdi_usb_close_internal (m_ftdi);
            ...
            if (index > 0)
            {
                ...
                continue;
						...
						res = ftdi_usb_open_dev(m_ftdi, dev);
            libusb_free_device_list(devs,1);
            return res;
        }
    }
...
/**
		Opens the m_ftdi-device described by a description-string.
    ...
    \retval -11: illegal description format
		\retval -12: m_ftdi context invalid
*/
int ftdi_usb_open_string(struct ftdi_context *m_ftdi, const char* description)
{
		if (m_ftdi == NULL)
				ftdi_error_return(-12, "m_ftdi context invalid");
    ...
    if (description[0] == 'd')
    {
        ...
        while ((dev = devs[i++]) != NULL)
        {
            ...
            {
								ret = ftdi_usb_open_dev(m_ftdi, dev);
                libusb_free_device_list(devs,1);
                return ret;
            }
        }
    ...
    else if (description[0] == 'i' || description[0] == 's')
    {
        ...
        if (description[0] == 's')
        {
            ...
            serial=endp+1;
				...
				return ftdi_usb_open_desc_index(m_ftdi, vendor, product, NULL, serial, index);
    }
    else
    {
        ftdi_error_return(-11, "illegal description format");
...
/**
		Resets the m_ftdi device.

		\param m_ftdi pointer to ftdi_context

    \retval  0: all fine
		\retval -1: m_ftdi reset failed
    \retval -2: USB device unavailable
*/
int ftdi_usb_reset(struct ftdi_context *m_ftdi)
{
		if (m_ftdi == NULL || m_ftdi->usb_dev == NULL)
        ftdi_error_return(-2, "USB device unavailable");

		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
                                SIO_RESET_REQUEST, SIO_RESET_SIO,
																m_ftdi->index, NULL, 0, m_ftdi->usb_write_timeout) < 0)
				ftdi_error_return(-1,"m_ftdi reset failed");
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_usb_purge_rx_buffer(struct ftdi_context *m_ftdi)
{
		if (m_ftdi == NULL || m_ftdi->usb_dev == NULL)
        ftdi_error_return(-2, "USB device unavailable");
		...
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
                                SIO_RESET_REQUEST, SIO_RESET_PURGE_RX,
																m_ftdi->index, NULL, 0, m_ftdi->usb_write_timeout) < 0)
				ftdi_error_return(-1, "m_ftdi purge of RX buffer failed");
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_usb_purge_tx_buffer(struct ftdi_context *m_ftdi)
{
		if (m_ftdi == NULL || m_ftdi->usb_dev == NULL)
        ftdi_error_return(-2, "USB device unavailable");
		...
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
                                SIO_RESET_REQUEST, SIO_RESET_PURGE_TX,
																m_ftdi->index, NULL, 0, m_ftdi->usb_write_timeout) < 0)
				ftdi_error_return(-1, "m_ftdi purge of TX buffer failed");
...
*/
int ftdi_usb_purge_buffers(struct ftdi_context *m_ftdi)
{
    int result;
		...
		result = ftdi_usb_purge_rx_buffer(m_ftdi);
    if (result < 0)
        return -1;

		result = ftdi_usb_purge_tx_buffer(m_ftdi);
    if (result < 0)
        return -2;
...
/**
		Closes the m_ftdi device. Call ftdi_deinit() if you're cleaning up.

		\param m_ftdi pointer to ftdi_context

    \retval  0: all fine
    \retval -1: usb_release failed
		\retval -3: m_ftdi context invalid
*/
int ftdi_usb_close(struct ftdi_context *m_ftdi)
{
    int rtn = 0;

		if (m_ftdi == NULL)
				ftdi_error_return(-3, "m_ftdi context invalid");

		if (m_ftdi->usb_dev != NULL)
				if (libusb_release_interface(m_ftdi->usb_dev, m_ftdi->interface) < 0)
            rtn = -1;
...
/**
    ...
    \internal
*/
static int ftdi_convert_baudrate(int baudrate, struct ftdi_context *m_ftdi,
 ...
 **/
int convert_baudrate_UT_export(int baudrate, struct ftdi_context *m_ftdi,
                               unsigned short *value, unsigned short *index)
{
		return ftdi_convert_baudrate(baudrate, m_ftdi, value, index);
}
...
/**
    ...
    \retval -3: USB device unavailable
*/
int ftdi_set_baudrate(struct ftdi_context *m_ftdi, int baudrate)
{
		...
		if (m_ftdi->bitbang_enabled)
    {
		...
		actual_baudrate = ftdi_convert_baudrate(baudrate, m_ftdi, &value, &index);
...
*/
int ftdi_set_line_property(struct ftdi_context *m_ftdi, enum ftdi_bits_type bits,
                           enum ftdi_stopbits_type sbit, enum ftdi_parity_type parity)
{
		return ftdi_set_line_property2(m_ftdi, bits, sbit, parity, BREAK_OFF);
}
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_set_line_property2(struct ftdi_context *m_ftdi, enum ftdi_bits_type bits,
                            enum ftdi_stopbits_type sbit, enum ftdi_parity_type parity,
...
*/
int ftdi_write_data(struct ftdi_context *m_ftdi, const unsigned char *buf, int size)
{
    int offset = 0;
    int actual_length;
    ...
    while (offset < size)
    {
				int write_size = m_ftdi->writebuffer_chunksize;

        if (offset+write_size > size)
            write_size = size-offset;

				if (libusb_bulk_transfer(m_ftdi->usb_dev, m_ftdi->in_ep, (unsigned char *)buf+offset, write_size, &actual_length, m_ftdi->usb_write_timeout) < 0)
...
static void LIBUSB_CALL ftdi_read_data_cb(struct libusb_transfer *transfer)
{
    struct ftdi_transfer_control *tc = (struct ftdi_transfer_control *) transfer->user_data;
		struct ftdi_context *m_ftdi = tc->m_ftdi;
    int packet_size, actual_length, num_of_chunks, chunk_remains, i, ret;

		packet_size = m_ftdi->max_packet_size;

    actual_length = transfer->actual_length;

    if (actual_length > 2)
    {
				// skip m_ftdi status bytes.
        // Maybe stored in the future to enable modem use
        num_of_chunks = actual_length / packet_size;
        chunk_remains = actual_length % packet_size;
				//printf("actual_length = %X, num_of_chunks = %X, chunk_remains = %X, readbuffer_offset = %X\n", actual_length, num_of_chunks, chunk_remains, m_ftdi->readbuffer_offset);

				m_ftdi->readbuffer_offset += 2;
        actual_length -= 2;

        if (actual_length > packet_size - 2)
        {
            for (i = 1; i < num_of_chunks; i++)
								memmove (m_ftdi->readbuffer+m_ftdi->readbuffer_offset+(packet_size - 2)*i,
												 m_ftdi->readbuffer+m_ftdi->readbuffer_offset+packet_size*i,
                         packet_size - 2);
            if (chunk_remains > 2)
            {
								memmove (m_ftdi->readbuffer+m_ftdi->readbuffer_offset+(packet_size - 2)*i,
        ...
        if (actual_length > 0)
        {
            ...
            {
								memcpy (tc->buf + tc->offset, m_ftdi->readbuffer + m_ftdi->readbuffer_offset, actual_length);
                ...
                if (tc->offset == tc->size)
                {
                    //printf("read_data exact rem %d offset %d\n",
										//m_ftdi->readbuffer_remaining, offset);
                    tc->completed = 1;
                    return;
                }
            }
            else
            {
                ...
                int part_size = tc->size - tc->offset;
								memcpy (tc->buf + tc->offset, m_ftdi->readbuffer + m_ftdi->readbuffer_offset, part_size);
                tc->offset += part_size;

								m_ftdi->readbuffer_offset += part_size;
								m_ftdi->readbuffer_remaining = actual_length - part_size;

                /* printf("Returning part: %d - size: %d - offset: %d - actual_length: %d - remaining: %d\n",
								part_size, size, offset, actual_length, m_ftdi->readbuffer_remaining); */
                tc->completed = 1;
                return;
            }
        }
...
struct ftdi_transfer_control *ftdi_write_data_submit(struct ftdi_context *m_ftdi, unsigned char *buf, int size)
{
    ...
    tc->offset = 0;
		...
		if (size < (int)m_ftdi->writebuffer_chunksize)
        write_size = size;
    else
				write_size = m_ftdi->writebuffer_chunksize;

		libusb_fill_bulk_transfer(transfer, m_ftdi->usb_dev, m_ftdi->in_ep, buf,
                              write_size, ftdi_write_data_cb, tc,
															m_ftdi->usb_write_timeout);
...
struct ftdi_transfer_control *ftdi_read_data_submit(struct ftdi_context *m_ftdi, unsigned char *buf, int size)
{
    struct ftdi_transfer_control *tc;
    struct libusb_transfer *transfer;
    int ret;
    ...
    tc->size = size;
		...
		if (size <= (int)m_ftdi->readbuffer_remaining)
    {
				memcpy (buf, m_ftdi->readbuffer+m_ftdi->readbuffer_offset, size);

        // Fix offsets
				m_ftdi->readbuffer_remaining -= size;
				m_ftdi->readbuffer_offset += size;

				/* printf("Returning bytes from buffer: %d - remaining: %d\n", size, m_ftdi->readbuffer_remaining); */
    ...
    tc->completed = 0;
		if (m_ftdi->readbuffer_remaining != 0)
    {
				memcpy (buf, m_ftdi->readbuffer+m_ftdi->readbuffer_offset, m_ftdi->readbuffer_remaining);

				tc->offset = m_ftdi->readbuffer_remaining;
    }
    else
        tc->offset = 0;
    ...
    if (!transfer)
    {
		...
		libusb_fill_bulk_transfer(transfer, m_ftdi->usb_dev, m_ftdi->out_ep, m_ftdi->readbuffer, m_ftdi->readbuffer_chunksize, ftdi_read_data_cb, tc, m_ftdi->usb_read_timeout);
    transfer->type = LIBUSB_TRANSFER_TYPE_BULK;
...
int ftdi_transfer_data_done(struct ftdi_transfer_control *tc)
{
    ...
    while (!tc->completed)
    {
				ret = libusb_handle_events(tc->m_ftdi->usb_ctx);
        if (ret < 0)
        {
            ...
            while (!tc->completed)
								if (libusb_handle_events(tc->m_ftdi->usb_ctx) < 0)
                    break;
    ...
    ret = tc->offset;
    /**
		 * tc->transfer could be NULL if "(size <= m_ftdi->readbuffer_remaining)"
...
/**
    ...
    \retval 0: all fine
		\retval -1: m_ftdi context invalid
*/
int ftdi_write_data_set_chunksize(struct ftdi_context *m_ftdi, unsigned int chunksize)
{
		if (m_ftdi == NULL)
				ftdi_error_return(-1, "m_ftdi context invalid");

		m_ftdi->writebuffer_chunksize = chunksize;
    return 0;
}
...
/**
    ...
    \retval 0: all fine
		\retval -1: m_ftdi context invalid
*/
int ftdi_write_data_get_chunksize(struct ftdi_context *m_ftdi, unsigned int *chunksize)
{
		if (m_ftdi == NULL)
				ftdi_error_return(-1, "m_ftdi context invalid");

		*chunksize = m_ftdi->writebuffer_chunksize;
    return 0;
}
...
*/
int ftdi_read_data(struct ftdi_context *m_ftdi, unsigned char *buf, int size)
{
    int offset = 0, ret, i, num_of_chunks, chunk_remains;
		int packet_size = m_ftdi->max_packet_size;
    int actual_length = 1;
		...
		if (m_ftdi->readbuffer_remaining != 0)
    {
        ...
        // Fix offset
				offset += m_ftdi->readbuffer_remaining;
    }
    ...
    while (offset < size && actual_length > 0)
    {
        ...
        /* returns how much received */
				ret = libusb_bulk_transfer (m_ftdi->usb_dev, m_ftdi->out_ep, m_ftdi->readbuffer, m_ftdi->readbuffer_chunksize, &actual_length, m_ftdi->usb_read_timeout);
        if (ret < 0)
        ...
        if (actual_length > 0)
        {
            ...
            {
								memcpy (buf+offset, m_ftdi->readbuffer+m_ftdi->readbuffer_offset, actual_length);
                //printf("buf[0] = %X, buf[1] = %X\n", buf[0], buf[1]);
                offset += actual_length;
            ...
            else
            {
                ...
                int part_size = size-offset;
								memcpy (buf+offset, m_ftdi->readbuffer+m_ftdi->readbuffer_offset, part_size);
...
/**
    ...
    \retval 0: all fine
		\retval -1: m_ftdi context invalid
*/
int ftdi_read_data_set_chunksize(struct ftdi_context *m_ftdi, unsigned int chunksize)
{
    ...
    if (chunksize > 16384)
		...
		if ((new_buf = (unsigned char *)realloc(m_ftdi->readbuffer, chunksize)) == NULL)
        ftdi_error_return(-1, "out of memory for readbuffer");

		m_ftdi->readbuffer = new_buf;
		m_ftdi->readbuffer_chunksize = chunksize;
...
/**
    ...
    \retval 0: all fine
		\retval -1: m_ftdi context invalid
*/
int ftdi_read_data_get_chunksize(struct ftdi_context *m_ftdi, unsigned int *chunksize)
{
		if (m_ftdi == NULL)
				ftdi_error_return(-1, "m_ftdi context invalid");

		*chunksize = m_ftdi->readbuffer_chunksize;
    return 0;
}
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_set_bitmode(struct ftdi_context *m_ftdi, unsigned char bitmask, unsigned char mode)
{
    ...
    usb_val |= (mode << 8);
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE, SIO_SET_BITMODE_REQUEST, usb_val, m_ftdi->index, NULL, 0, m_ftdi->usb_write_timeout) < 0)
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_disable_bitbang(struct ftdi_context *m_ftdi)
{
		if (m_ftdi == NULL || m_ftdi->usb_dev == NULL)
        ftdi_error_return(-2, "USB device unavailable");

		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE, SIO_SET_BITMODE_REQUEST, 0, m_ftdi->index, NULL, 0, m_ftdi->usb_write_timeout) < 0)
        ftdi_error_return(-1, "unable to leave bitbang mode. Perhaps not a BM type chip?");

		m_ftdi->bitbang_enabled = 0;
    return 0;
}
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_read_pins(struct ftdi_context *m_ftdi, unsigned char *pins)
{
		if (m_ftdi == NULL || m_ftdi->usb_dev == NULL)
        ftdi_error_return(-2, "USB device unavailable");

		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE, SIO_READ_PINS_REQUEST, 0, m_ftdi->index, (unsigned char *)pins, 1, m_ftdi->usb_read_timeout) != 1)
...
/**
    Set latency timer
		...
		The m_ftdi chip keeps data in the internal buffer for a specific
    ...
    \retval -3: USB device unavailable
*/
int ftdi_set_latency_timer(struct ftdi_context *m_ftdi, unsigned char latency)
{
    unsigned short usb_val;
    ...
    usb_val = latency;
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE, SIO_SET_LATENCY_TIMER_REQUEST, usb_val, m_ftdi->index, NULL, 0, m_ftdi->usb_write_timeout) < 0)
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_get_latency_timer(struct ftdi_context *m_ftdi, unsigned char *latency)
{
    unsigned short usb_val;
		...
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE, SIO_GET_LATENCY_TIMER_REQUEST, 0, m_ftdi->index, (unsigned char *)&usb_val, 1, m_ftdi->usb_read_timeout) != 1)
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_poll_modem_status(struct ftdi_context *m_ftdi, unsigned short *status)
{
    char usb_val[2];
		...
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE, SIO_POLL_MODEM_STATUS_REQUEST, 0, m_ftdi->index, (unsigned char *)usb_val, 2, m_ftdi->usb_read_timeout) != 2)
...
/**
		Set flowcontrol for m_ftdi chip
    ...
    \retval -2: USB device unavailable
*/
int ftdi_setflowctrl(struct ftdi_context *m_ftdi, int flowctrl)
{
		if (m_ftdi == NULL || m_ftdi->usb_dev == NULL)
        ftdi_error_return(-2, "USB device unavailable");

		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
																SIO_SET_FLOW_CTRL_REQUEST, 0, (flowctrl | m_ftdi->index),
																NULL, 0, m_ftdi->usb_write_timeout) < 0)
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_setdtr(struct ftdi_context *m_ftdi, int state)
{
    unsigned short usb_val;
    ...
    if (state)
		...
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
																SIO_SET_MODEM_CTRL_REQUEST, usb_val, m_ftdi->index,
																NULL, 0, m_ftdi->usb_write_timeout) < 0)
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_setrts(struct ftdi_context *m_ftdi, int state)
{
    unsigned short usb_val;
...
/**
    ...
    \retval -2: USB device unavailable
 */
int ftdi_setdtr_rts(struct ftdi_context *m_ftdi, int dtr, int rts)
{
    unsigned short usb_val;
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_set_event_char(struct ftdi_context *m_ftdi,
                        unsigned char eventch, unsigned char enable)
{
    ...
    if (enable)
		...
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE, SIO_SET_EVENT_CHAR_REQUEST, usb_val, m_ftdi->index, NULL, 0, m_ftdi->usb_write_timeout) < 0)
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_set_error_char(struct ftdi_context *m_ftdi,
                        unsigned char errorch, unsigned char enable)
{
    ...
    if (enable)
		...
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE, SIO_SET_ERROR_CHAR_REQUEST, usb_val, m_ftdi->index, NULL, 0, m_ftdi->usb_write_timeout) < 0)
...
*/
int ftdi_eeprom_initdefaults(struct ftdi_context *m_ftdi, char * manufacturer,
                             char * product, char * serial)
{
    ...
    eeprom->use_serial = 1;
		if ((m_ftdi->type == TYPE_AM) || (m_ftdi->type == TYPE_BM) ||
						(m_ftdi->type == TYPE_R))
        eeprom->product_id = 0x6001;
		else if (m_ftdi->type == TYPE_4232H)
        eeprom->product_id = 0x6011;
		else if (m_ftdi->type == TYPE_232H)
        eeprom->product_id = 0x6014;
		else if (m_ftdi->type == TYPE_230X)
        eeprom->product_id = 0x6015;
    else
        eeprom->product_id = 0x6010;

		if (m_ftdi->type == TYPE_AM)
...
int ftdi_eeprom_set_strings(struct ftdi_context *m_ftdi, char * manufacturer,
                            char * product, char * serial)
{
    struct ftdi_eeprom *eeprom;
...
/**
    ...
    \retval -1: eeprom size (128 bytes) exceeded by custom strings
		\retval -2: Invalid eeprom or m_ftdi pointer
...
*/
int ftdi_eeprom_build(struct ftdi_context *m_ftdi)
{
    ...
    // empty eeprom
		if (m_ftdi->type == TYPE_230X)
    {
        /* FT230X have a reserved section in the middle of the MTP,
           which cannot be written to, but must be included in the checksum */
				memset(m_ftdi->eeprom->buf, 0, 0x80);
				memset((m_ftdi->eeprom->buf + 0xa0), 0, (FTDI_MAX_EEPROM_SIZE - 0xa0));
    }
    else
    {
				memset(m_ftdi->eeprom->buf, 0, FTDI_MAX_EEPROM_SIZE);
    }
    ...
    output[0x09] = eeprom->max_power / MAX_POWER_MILLIAMP_PER_UNIT;
		...
		if ((m_ftdi->type != TYPE_AM) && (m_ftdi->type != TYPE_230X))
    {
    ...
    output[0x13] = serial_size*2 + 2;
		...
		if (m_ftdi->type > TYPE_AM) /* use_serial not used in AM devices */
    {
    ...
    {
				if ((m_ftdi->type == TYPE_230X) && (i == 0x12))
        {
            /* FT230X has a user section in the MTP which is not part of the checksum */
            i = 0x40;
        }
				if ((m_ftdi->type == TYPE_230X) && (i >=  0x40) && (i < 0x50)) {
            uint16_t data;
						if (ftdi_read_eeprom_location(m_ftdi, i, &data)) {
...
/* Decode the encoded EEPROM field for the m_ftdi Mode into a value for the abstracted
 * EEPROM structure
 *
...
*/
int ftdi_eeprom_decode(struct ftdi_context *m_ftdi, int verbose)
{
    int i, j;
		...
		if ((m_ftdi->type == TYPE_AM) || (m_ftdi->type == TYPE_BM))
    {
		...
		else if (m_ftdi->type == TYPE_2232C)
    {
		...
		else if (m_ftdi->type == TYPE_R)
    {
        ...
        if ( (buf[0x01]&0x40) != 0x40)
            fprintf(stderr,
                    ...
                    " If this happened with the\n"
										" EEPROM programmed by m_ftdi tools, please report "
		...
		else if ((m_ftdi->type == TYPE_2232H) || (m_ftdi->type == TYPE_4232H))
    {
        eeprom->channel_a_driver = buf[0x00] & DRIVER_VCP;
        eeprom->channel_b_driver = buf[0x01] & DRIVER_VCP;

				if (m_ftdi->type == TYPE_2232H)
        {
            eeprom->channel_a_type   = bit2type(buf[0x00] & 0x7);
    ...
    if (verbose)
    {
				...
				if (m_ftdi->type == TYPE_232H)
        {
            ...
            fprintf(stdout,"Channel B has Mode %s%s%s\n",
				...
				if (((m_ftdi->type == TYPE_BM) || (m_ftdi->type == TYPE_2232C)) &&
...
*/
int ftdi_get_eeprom_value(struct ftdi_context *m_ftdi, enum ftdi_eeprom_value value_name, int* value)
{
    switch (value_name)
    {
        case VENDOR_ID:
						*value = m_ftdi->eeprom->vendor_id;
            break;
        case PRODUCT_ID:
						*value = m_ftdi->eeprom->product_id;
            break;
        case RELEASE_NUMBER:
						*value = m_ftdi->eeprom->release_number;
            break;
        case SELF_POWERED:
						*value = m_ftdi->eeprom->self_powered;
            break;
        case REMOTE_WAKEUP:
						*value = m_ftdi->eeprom->remote_wakeup;
            break;
        case IS_NOT_PNP:
						*value = m_ftdi->eeprom->is_not_pnp;
            break;
        case SUSPEND_DBUS7:
						*value = m_ftdi->eeprom->suspend_dbus7;
            break;
        case IN_IS_ISOCHRONOUS:
						*value = m_ftdi->eeprom->in_is_isochronous;
            break;
        case OUT_IS_ISOCHRONOUS:
						*value = m_ftdi->eeprom->out_is_isochronous;
            break;
        case SUSPEND_PULL_DOWNS:
						*value = m_ftdi->eeprom->suspend_pull_downs;
            break;
        case USE_SERIAL:
						*value = m_ftdi->eeprom->use_serial;
            break;
        case USB_VERSION:
						*value = m_ftdi->eeprom->usb_version;
            break;
        case USE_USB_VERSION:
						*value = m_ftdi->eeprom->use_usb_version;
            break;
        case MAX_POWER:
						*value = m_ftdi->eeprom->max_power;
            break;
        case CHANNEL_A_TYPE:
						*value = m_ftdi->eeprom->channel_a_type;
            break;
        case CHANNEL_B_TYPE:
						*value = m_ftdi->eeprom->channel_b_type;
            break;
        case CHANNEL_A_DRIVER:
						*value = m_ftdi->eeprom->channel_a_driver;
            break;
        case CHANNEL_B_DRIVER:
						*value = m_ftdi->eeprom->channel_b_driver;
            break;
        case CHANNEL_C_DRIVER:
						*value = m_ftdi->eeprom->channel_c_driver;
            break;
        case CHANNEL_D_DRIVER:
						*value = m_ftdi->eeprom->channel_d_driver;
            break;
        case CHANNEL_A_RS485:
						*value = m_ftdi->eeprom->channel_a_rs485enable;
            break;
        case CHANNEL_B_RS485:
						*value = m_ftdi->eeprom->channel_b_rs485enable;
            break;
        case CHANNEL_C_RS485:
						*value = m_ftdi->eeprom->channel_c_rs485enable;
            break;
        case CHANNEL_D_RS485:
						*value = m_ftdi->eeprom->channel_d_rs485enable;
            break;
        case CBUS_FUNCTION_0:
						*value = m_ftdi->eeprom->cbus_function[0];
            break;
        case CBUS_FUNCTION_1:
						*value = m_ftdi->eeprom->cbus_function[1];
            break;
        case CBUS_FUNCTION_2:
						*value = m_ftdi->eeprom->cbus_function[2];
            break;
        case CBUS_FUNCTION_3:
						*value = m_ftdi->eeprom->cbus_function[3];
            break;
        case CBUS_FUNCTION_4:
						*value = m_ftdi->eeprom->cbus_function[4];
            break;
        case CBUS_FUNCTION_5:
						*value = m_ftdi->eeprom->cbus_function[5];
            break;
        case CBUS_FUNCTION_6:
						*value = m_ftdi->eeprom->cbus_function[6];
            break;
        case CBUS_FUNCTION_7:
						*value = m_ftdi->eeprom->cbus_function[7];
            break;
        case CBUS_FUNCTION_8:
						*value = m_ftdi->eeprom->cbus_function[8];
            break;
        case CBUS_FUNCTION_9:
						*value = m_ftdi->eeprom->cbus_function[8];
            break;
        case HIGH_CURRENT:
						*value = m_ftdi->eeprom->high_current;
            break;
        case HIGH_CURRENT_A:
						*value = m_ftdi->eeprom->high_current_a;
            break;
        case HIGH_CURRENT_B:
						*value = m_ftdi->eeprom->high_current_b;
            break;
        case INVERT:
						*value = m_ftdi->eeprom->invert;
            break;
        case GROUP0_DRIVE:
						*value = m_ftdi->eeprom->group0_drive;
            break;
        case GROUP0_SCHMITT:
						*value = m_ftdi->eeprom->group0_schmitt;
            break;
        case GROUP0_SLEW:
						*value = m_ftdi->eeprom->group0_slew;
            break;
        case GROUP1_DRIVE:
						*value = m_ftdi->eeprom->group1_drive;
            break;
        case GROUP1_SCHMITT:
						*value = m_ftdi->eeprom->group1_schmitt;
            break;
        case GROUP1_SLEW:
						*value = m_ftdi->eeprom->group1_slew;
            break;
        case GROUP2_DRIVE:
						*value = m_ftdi->eeprom->group2_drive;
            break;
        case GROUP2_SCHMITT:
						*value = m_ftdi->eeprom->group2_schmitt;
            break;
        case GROUP2_SLEW:
						*value = m_ftdi->eeprom->group2_slew;
            break;
        case GROUP3_DRIVE:
						*value = m_ftdi->eeprom->group3_drive;
            break;
        case GROUP3_SCHMITT:
						*value = m_ftdi->eeprom->group3_schmitt;
            break;
        case GROUP3_SLEW:
						*value = m_ftdi->eeprom->group3_slew;
            break;
        case POWER_SAVE:
						*value = m_ftdi->eeprom->powersave;
            break;
        case CLOCK_POLARITY:
						*value = m_ftdi->eeprom->clock_polarity;
            break;
        case DATA_ORDER:
						*value = m_ftdi->eeprom->data_order;
            break;
        case FLOW_CONTROL:
						*value = m_ftdi->eeprom->flow_control;
            break;
        case CHIP_TYPE:
						*value = m_ftdi->eeprom->chip;
            break;
        case CHIP_SIZE:
						*value = m_ftdi->eeprom->size;
            break;
        default:
...
*/
int ftdi_set_eeprom_value(struct ftdi_context *m_ftdi, enum ftdi_eeprom_value value_name, int value)
{
    switch (value_name)
    {
        case VENDOR_ID:
						m_ftdi->eeprom->vendor_id = value;
            break;
        case PRODUCT_ID:
						m_ftdi->eeprom->product_id = value;
            break;
        case RELEASE_NUMBER:
						m_ftdi->eeprom->release_number = value;
            break;
        case SELF_POWERED:
						m_ftdi->eeprom->self_powered = value;
            break;
        case REMOTE_WAKEUP:
						m_ftdi->eeprom->remote_wakeup = value;
            break;
        case IS_NOT_PNP:
						m_ftdi->eeprom->is_not_pnp = value;
            break;
        case SUSPEND_DBUS7:
						m_ftdi->eeprom->suspend_dbus7 = value;
            break;
        case IN_IS_ISOCHRONOUS:
						m_ftdi->eeprom->in_is_isochronous = value;
            break;
        case OUT_IS_ISOCHRONOUS:
						m_ftdi->eeprom->out_is_isochronous = value;
            break;
        case SUSPEND_PULL_DOWNS:
						m_ftdi->eeprom->suspend_pull_downs = value;
            break;
        case USE_SERIAL:
						m_ftdi->eeprom->use_serial = value;
            break;
        case USB_VERSION:
						m_ftdi->eeprom->usb_version = value;
            break;
        case USE_USB_VERSION:
						m_ftdi->eeprom->use_usb_version = value;
            break;
        case MAX_POWER:
						m_ftdi->eeprom->max_power = value;
            break;
        case CHANNEL_A_TYPE:
						m_ftdi->eeprom->channel_a_type = value;
            break;
        case CHANNEL_B_TYPE:
						m_ftdi->eeprom->channel_b_type = value;
            break;
        case CHANNEL_A_DRIVER:
						m_ftdi->eeprom->channel_a_driver = value;
            break;
        case CHANNEL_B_DRIVER:
						m_ftdi->eeprom->channel_b_driver = value;
            break;
        case CHANNEL_C_DRIVER:
						m_ftdi->eeprom->channel_c_driver = value;
            break;
        case CHANNEL_D_DRIVER:
						m_ftdi->eeprom->channel_d_driver = value;
            break;
        case CHANNEL_A_RS485:
						m_ftdi->eeprom->channel_a_rs485enable = value;
            break;
        case CHANNEL_B_RS485:
						m_ftdi->eeprom->channel_b_rs485enable = value;
            break;
        case CHANNEL_C_RS485:
						m_ftdi->eeprom->channel_c_rs485enable = value;
            break;
        case CHANNEL_D_RS485:
						m_ftdi->eeprom->channel_d_rs485enable = value;
            break;
        case CBUS_FUNCTION_0:
						m_ftdi->eeprom->cbus_function[0] = value;
            break;
        case CBUS_FUNCTION_1:
						m_ftdi->eeprom->cbus_function[1] = value;
            break;
        case CBUS_FUNCTION_2:
						m_ftdi->eeprom->cbus_function[2] = value;
            break;
        case CBUS_FUNCTION_3:
						m_ftdi->eeprom->cbus_function[3] = value;
            break;
        case CBUS_FUNCTION_4:
						m_ftdi->eeprom->cbus_function[4] = value;
            break;
        case CBUS_FUNCTION_5:
						m_ftdi->eeprom->cbus_function[5] = value;
            break;
        case CBUS_FUNCTION_6:
						m_ftdi->eeprom->cbus_function[6] = value;
            break;
        case CBUS_FUNCTION_7:
						m_ftdi->eeprom->cbus_function[7] = value;
            break;
        case CBUS_FUNCTION_8:
						m_ftdi->eeprom->cbus_function[8] = value;
            break;
        case CBUS_FUNCTION_9:
						m_ftdi->eeprom->cbus_function[9] = value;
            break;
        case HIGH_CURRENT:
						m_ftdi->eeprom->high_current = value;
            break;
        case HIGH_CURRENT_A:
						m_ftdi->eeprom->high_current_a = value;
            break;
        case HIGH_CURRENT_B:
						m_ftdi->eeprom->high_current_b = value;
            break;
        case INVERT:
						m_ftdi->eeprom->invert = value;
            break;
        case GROUP0_DRIVE:
						m_ftdi->eeprom->group0_drive = value;
            break;
        case GROUP0_SCHMITT:
						m_ftdi->eeprom->group0_schmitt = value;
            break;
        case GROUP0_SLEW:
						m_ftdi->eeprom->group0_slew = value;
            break;
        case GROUP1_DRIVE:
						m_ftdi->eeprom->group1_drive = value;
            break;
        case GROUP1_SCHMITT:
						m_ftdi->eeprom->group1_schmitt = value;
            break;
        case GROUP1_SLEW:
						m_ftdi->eeprom->group1_slew = value;
            break;
        case GROUP2_DRIVE:
						m_ftdi->eeprom->group2_drive = value;
            break;
        case GROUP2_SCHMITT:
						m_ftdi->eeprom->group2_schmitt = value;
            break;
        case GROUP2_SLEW:
						m_ftdi->eeprom->group2_slew = value;
            break;
        case GROUP3_DRIVE:
						m_ftdi->eeprom->group3_drive = value;
            break;
        case GROUP3_SCHMITT:
						m_ftdi->eeprom->group3_schmitt = value;
            break;
        case GROUP3_SLEW:
						m_ftdi->eeprom->group3_slew = value;
            break;
        case CHIP_TYPE:
						m_ftdi->eeprom->chip = value;
            break;
        case POWER_SAVE:
						m_ftdi->eeprom->powersave = value;
            break;
        case CLOCK_POLARITY:
						m_ftdi->eeprom->clock_polarity = value;
            break;
        case DATA_ORDER:
						m_ftdi->eeprom->data_order = value;
            break;
        case FLOW_CONTROL:
						m_ftdi->eeprom->flow_control = value;
            break;
        case CHIP_SIZE:
...
*/
int ftdi_get_eeprom_buf(struct ftdi_context *m_ftdi, unsigned char * buf, int size)
{
		if (!m_ftdi || !(m_ftdi->eeprom))
        ftdi_error_return(-1, "No appropriate structure");

		if (!buf || size < m_ftdi->eeprom->size)
    ...
    if (size > FTDI_MAX_EEPROM_SIZE)
		...
		memcpy(buf, m_ftdi->eeprom->buf, size);
...
*/
int ftdi_set_eeprom_buf(struct ftdi_context *m_ftdi, const unsigned char * buf, int size)
{
		if (!m_ftdi || !(m_ftdi->eeprom) || !buf)
        ftdi_error_return(-1, "No appropriate structure");
    ...
    if (size > FTDI_MAX_EEPROM_SIZE)
		...
		memcpy(m_ftdi->eeprom->buf, buf, size);
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_read_eeprom_location (struct ftdi_context *m_ftdi, int eeprom_addr, unsigned short *eeprom_val)
{
		if (m_ftdi == NULL || m_ftdi->usb_dev == NULL)
        ftdi_error_return(-2, "USB device unavailable");

		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE, SIO_READ_EEPROM_REQUEST, 0, eeprom_addr, (unsigned char *)eeprom_val, 2, m_ftdi->usb_read_timeout) != 2)
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_read_eeprom(struct ftdi_context *m_ftdi)
{
    int i;
    unsigned char *buf;
    ...
    for (i = 0; i < FTDI_MAX_EEPROM_SIZE/2; i++)
    {
        if (libusb_control_transfer(
										m_ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE,SIO_READ_EEPROM_REQUEST, 0, i,
										buf+(i*2), 2, m_ftdi->usb_read_timeout) != 2)
            ftdi_error_return(-1, "reading eeprom failed");
    }

		if (m_ftdi->type == TYPE_R)
				m_ftdi->eeprom->size = 0x80;
...
/**
    ...
    \retval -2: USB device unavailable
*/
int ftdi_read_chipid(struct ftdi_context *m_ftdi, unsigned int *chipid)
{
    unsigned int a = 0, b = 0;
		...
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE, SIO_READ_EEPROM_REQUEST, 0, 0x43, (unsigned char *)&a, 2, m_ftdi->usb_read_timeout) == 2)
    {
        a = a << 8 | a >> 8;
				if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE, SIO_READ_EEPROM_REQUEST, 0, 0x44, (unsigned char *)&b, 2, m_ftdi->usb_read_timeout) == 2)
        {
            b = b << 8 | b >> 8;
            a = (a << 16) | (b & 0xFFFF);
...
*/
int ftdi_write_eeprom_location(struct ftdi_context *m_ftdi, int eeprom_addr,
                               unsigned short eeprom_val)
{
		...
		switch (m_ftdi->type)
    {
		...
		if (ftdi_read_eeprom_location( m_ftdi, chip_type_location>>1, &chip_type))
...
*/
int ftdi_write_eeprom(struct ftdi_context *m_ftdi)
{
    ...
    unsigned char *eeprom;
		...
		if(m_ftdi->eeprom->initialized_for_connected_device == 0)
        ftdi_error_return(-3, "EEPROM not initialized for the connected device");

		eeprom = m_ftdi->eeprom->buf;

    /* These commands were traced while running MProg */
		if ((ret = ftdi_usb_reset(m_ftdi)) != 0)
        return ret;
		if ((ret = ftdi_poll_modem_status(m_ftdi, &status)) != 0)
        return ret;
		if ((ret = ftdi_set_latency_timer(m_ftdi, 0x77)) != 0)
        return ret;

		for (i = 0; i < m_ftdi->eeprom->size/2; i++)
    {
        /* Do not try to write to reserved area */
				if ((m_ftdi->type == TYPE_230X) && (i == 0x40))
        {
            i = 0x50;
        }
        usb_val = eeprom[i*2];
...
/**
    Erase eeprom
		...
		This is not supported on FT232R/FT245R according to the MProg manual from m_ftdi.
...
*/
#define MAGIC 0x55aa
int ftdi_erase_eeprom(struct ftdi_context *m_ftdi)
{
    unsigned short eeprom_value;
		if (m_ftdi == NULL || m_ftdi->usb_dev == NULL)
        ftdi_error_return(-2, "USB device unavailable");

		if ((m_ftdi->type == TYPE_R) || (m_ftdi->type == TYPE_230X))
    {
				m_ftdi->eeprom->chip = 0;
        return 0;
    }

		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE, SIO_ERASE_EEPROM_REQUEST,
																0, 0, NULL, 0, m_ftdi->usb_write_timeout) < 0)
    ...
    /* detect chip type by writing 0x55AA as magic at word position 0xc0
       ...
       Chip is 93x66 if magic is only read at word position 0xc0*/
		...
		if (ftdi_read_eeprom_location( m_ftdi, 0x00, &eeprom_value))
        ftdi_error_return(-4, "Reading failed");
    if (eeprom_value == MAGIC)
    {
				m_ftdi->eeprom->chip = 0x46;
    }
    else
    {
				if (ftdi_read_eeprom_location( m_ftdi, 0x40, &eeprom_value))
            ftdi_error_return(-4, "Reading failed");
        if (eeprom_value == MAGIC)
						m_ftdi->eeprom->chip = 0x56;
        else
        {
						if (ftdi_read_eeprom_location( m_ftdi, 0xc0, &eeprom_value))
                ftdi_error_return(-4, "Reading failed");
            if (eeprom_value == MAGIC)
								m_ftdi->eeprom->chip = 0x66;
            else
            {
								m_ftdi->eeprom->chip = -1;
            }
        }
    }
		if (libusb_control_transfer(m_ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE, SIO_ERASE_EEPROM_REQUEST,
...
*/
char *ftdi_get_error_string (struct ftdi_context *m_ftdi)
{
		if (m_ftdi == NULL)
        return "";

		return m_ftdi->error_str;
}
